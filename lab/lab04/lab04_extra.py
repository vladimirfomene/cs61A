...
def put_piece(board, max_rows, column, player):
    ...
    while num_row >= 0:
        if board[num_row][column] == '-':
            board[num_row] = replace_elem(board[num_row], column, player)
            row_index = num_row
            break
        num_row -= 1
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    player_occurence = 0
    for j in range(max_cols):
        if player_occurence == num_connect:
            return True
        if player == get_piece(board, row, j):
            player_occurence += 1
        else:
            player_occurence = 0
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for j in range(max_rows):
        if player_occurence == num_connect:
            return True
        if player == get_piece(board, j, col):
            player_occurence += 1
        else:
            player_occurence = 0
    return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    return diagonal_win or check_win_column(board, max_rows, max_cols,
            num_connect, col, player) or check_win_row(board, max_rows,
                    max_cols, num_connect, row, player)
